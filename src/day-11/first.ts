...
const input = readFileSync(path.join(__dirname, '../../src/day-11', 'input.txt'), 'utf-8').split('\n')
...
function findGalaxies(): Galaxy[] {
  ...
  for (const [y, line] of input.entries()) {
    for (let x = 0; x < line.length; x++) if (line[x] === '#') galaxies.push({ x, y })
  }
...
function searchForExpand(): { columns: number[]; rows: number[] } {
  ...
  for (let x = 0; x < input[0].length; x++)
    if (input[0][x] === '.') {
      let y = 1
      for (; y < input.length; y++) if (input[y][x] !== '.') break
      if (y === input.length) columns.push(x)
    }

  for (let y = 0; y < input.length; y++) if (input[y].includes('#') === false) rows.push(y)

  return { columns, rows }
}

function expand(galaxies: Galaxy[], toExpand: { columns: number[]; rows: number[] }, times: number = 1) {
  // Expand columns
  ...
  columns.forEach((column) => {
    ...
    for (; idx < galaxies.length; idx++) galaxies[idx].x += times - 1
  })
  ...
  rows.forEach((row) => {
    ...
    for (; idx < galaxies.length; idx++) galaxies[idx].y += times - 1
  })
}
