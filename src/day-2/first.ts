...
function findGameSet(game: GameSet): number | undefined {
  for (const gameSet of game.sets) {
    if (gameSet.blue > toMatchSet.blue || gameSet.green > toMatchSet.green || gameSet.red > toMatchSet.red) return
  }
...
export function first() {
  ...
  for (const possibleGame of possibleGames) initValue += possibleGame !== undefined ? possibleGame : 0
