...
const input = readFileSync(path.join(__dirname, '../../src/day-5', 'input.txt'), 'utf-8').split('\n')
...
function seedAlreadyChecked(seed: number, alreadyCheckedSeedPairs: number[]): boolean {
  for (let seedPairCount = 0; seedPairCount < alreadyCheckedSeedPairs.length; seedPairCount += 2) {
    const startSeed = alreadyCheckedSeedPairs[seedPairCount]
    const seedCount = alreadyCheckedSeedPairs[seedPairCount + 1]

    if (seed >= startSeed && seed < startSeed + seedCount) return true
  }
...
function mapSeeds() {
  ...
  const alreadyCheckedSeedPairs: number[] = []

  for (let seedPairCount = 0; seedPairCount < seeds.length; seedPairCount += 2) {
    const startSeed = seeds[seedPairCount]
    const seedCount = seeds[seedPairCount + 1]

    for (let cnt = 0; cnt < seedCount; cnt++) {
      const seed = startSeed + cnt

      if (seedAlreadyChecked(seed, alreadyCheckedSeedPairs)) continue
    ...
    alreadyCheckedSeedPairs.push(startSeed, seedCount)
  }
